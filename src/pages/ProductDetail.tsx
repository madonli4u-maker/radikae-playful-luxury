      ...
      <div className="container mx-auto px-4 py-6">
        ...
        <div className="grid md:grid-cols-2 gap-8 md:gap-12">
          ...
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5 }}>
            <div className={`relative aspect-square overflow-hidden mb-4 ${isJewelry ? 'rounded-xl' : 'rounded-2xl'}`}>
              ...
              {product.badge && (
                <span className={`absolute top-4 left-4 px-3 py-1.5 rounded-full text-xs font-semibold ${isJewelry ? 'bg-primary text-primary-foreground' : 'bg-toy-pink text-white'}`}>
                  {product.badge}
                </span>
              )}
            </div>
            <div className="grid grid-cols-4 gap-2">
              {galleryImages.map((img, i) => (
                <button key={i} onClick={() => setSelectedImage(i)}
                  className={`aspect-square overflow-hidden rounded-lg border-2 transition-all ${selectedImage === i ? (isJewelry ? 'border-primary' : 'border-toy-pink') : 'border-transparent hover:border-border'}`}>
          ...
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5, delay: 0.1 }} className="flex flex-col">
            <p className={`text-xs uppercase tracking-[0.2em] mb-2 ${isJewelry ? 'text-secondary' : 'text-toy-pink'}`}>{product.subcategory}</p>
            <h1 className={`font-serif font-bold mb-4 ${isJewelry ? 'text-3xl md:text-4xl gold-shimmer inline-block' : 'text-2xl md:text-3xl text-foreground'}`}>{product.name}</h1>
            ...
            <div className="flex items-baseline gap-3 mb-6">
              <span className={`text-3xl font-bold ${isJewelry ? 'gold-text font-serif' : 'text-foreground'}`}>₹{product.price.toLocaleString('en-IN')}</span>
            ...
            <p className={`text-sm leading-relaxed mb-6 ${isJewelry ? 'text-secondary' : 'text-muted-foreground'}`}>{product.description}</p>

            <div className="flex items-center gap-4 mb-6">
              <span className="text-sm font-medium text-muted-foreground">Quantity</span>
              <div className="flex items-center border border-border rounded-lg">
                <button onClick={() => setQuantity(q => Math.max(1, q - 1))} className="p-2 hover:bg-muted transition-colors"><Minus className="w-4 h-4" /></button>
                <span className="px-4 text-sm font-semibold">{quantity}</span>
                <button onClick={() => setQuantity(q => q + 1)} className="p-2 hover:bg-muted transition-colors"><Plus className="w-4 h-4" /></button>
              </div>
            </div>

            <div className="flex gap-3 mb-8">
              <button onClick={handleAddToCart}
                className={`flex-1 flex items-center justify-center gap-2 py-3.5 rounded-xl font-semibold text-sm transition-all ${isJewelry ? 'bg-primary text-primary-foreground hover:brightness-110' : 'bg-toy-pink text-white hover:brightness-110'}`}>
                <ShoppingCart className="w-4 h-4" /> Add to Cart
              </button>
              <button onClick={() => toggleWishlist(product)}
                className={`p-3.5 rounded-xl border transition-all ${isInWishlist(product.id) ? (isJewelry ? 'border-primary bg-primary/10' : 'border-toy-pink bg-toy-pink/10') : 'border-border hover:border-muted-foreground'}`}>
        ...
        {similarBySubcategory.length > 0 && (
          <section className="mt-16 pb-4">
            <h2 className={`font-serif text-xl md:text-2xl font-bold mb-6 ${isJewelry ? 'gold-shimmer inline-block' : 'text-foreground'}`}>Similar Products</h2>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-5">
              {similarBySubcategory.map(p => <ProductCard key={p.id} product={p} variant={isJewelry ? 'jewelry' : 'toy'} />)}
            </div>
          </section>
        )}
        ...
        {related.length > 0 && (
          <section className="mt-12 pb-12">
            <h2 className={`font-serif text-xl md:text-2xl font-bold mb-6 ${isJewelry ? 'gold-shimmer inline-block' : 'text-foreground'}`}>You May Also Like</h2>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-5">
              {related.map(p => <ProductCard key={p.id} product={p} variant={isJewelry ? 'jewelry' : 'toy'} />)}
            </div>
          </section>
        )}
      </div>
